...
function loadTask() {
  ...
  const promptFile = resolve(__dirname, 'test_prompt.md');
...
async function main() {
  ...
  log('INIT', `Task: ${TASK.slice(0, 100)}...`);
  ...
  const systemPrompt = [
    'You are controlling a remote Linux desktop (LXDE) via VNC KVM tools.',
    'Complete the given task efficiently.',
    'Use the vnc_command tool with appropriate actions.',
    'Take a screenshot first to see the screen state.',
        ...
        try {
          ...
          for (const item of result.content) {
            if (item.type === 'text') {
              ...
              log('RESULT', `[image ${item.mimeType || 'image/png'}]`);
