...
// The daemon branch carries no MCP server source; the published claude-kvm
// package provides it (installed via test/slow/package.json). The daemon
// binary itself is injected through CLAUDE_KVM_DAEMON_PATH so the locally
// built (from-source) daemon is what actually gets exercised.
const indexPath = process.env.MCP_SERVER_PATH
  || resolve(__dirname, '..', 'node_modules', 'claude-kvm', 'index.js');
