...
export class VNCClient extends EventEmitter {
   ...
   * @param {number} [options.connectTimeoutMs] - TCP connect timeout (default 10000)
   * @param {number} [options.screenshotTimeoutMs] - Screenshot frame wait timeout (default 3000)
   */
  constructor(vncConfig, options = {}) {
    super();
    ...
    this.screenshotTimeoutMs = options.screenshotTimeoutMs ?? 3000;
  ...
  /**
   * Internal connect — creates TCP socket, performs RFB handshake.
   * Extracted so reconnect can reuse the same logic.
   * @returns {Promise<import('./types').VNCServerInfo>}
   */
  _doConnect() {
    return new Promise((resolve, reject) => {
      // Reset FSM state for fresh handshake
      this._buffer = Buffer.alloc(0);
      this._state = 'version';
      this._expectedBytes = 12;
      this._vencryptSubType = 0;
      ...
      this.socket.on('timeout', () => {
        dbg('TIMEOUT', `TCP connect timeout after ${this.connectTimeoutMs}ms`);
        this.socket.destroy();
        reject(new Error(`VNC connect timeout after ${this.connectTimeoutMs}ms`));
      });

      this.socket.on('connect', () => {
        this.connected = true;
        this.socket.setTimeout(0); // clear timeout after TCP connect
      ...
      this.socket.on('close', () => {
        ...
        this.emit('close');
        // Auto-reconnect if connection was established and not intentionally closed
        if (wasReady && !this._intentionalDisconnect) {
  ...
  async _scheduleReconnect() {
    if (this._reconnectAttempts >= this._maxReconnectAttempts) {
      dbg('RECONNECT', `giving up after ${this._maxReconnectAttempts} attempts`);
      this.reconnecting = false;
    ...
    dbg('RECONNECT', `attempt ${this._reconnectAttempts}/${this._maxReconnectAttempts} in ${delay}ms`);

    await new Promise(r => setTimeout(r, delay));

    try {
      // Clean up stale listeners before reconnecting
      this.removeAllListeners('ready');
      this.removeAllListeners('handshake-error');

      await this._doConnect();
      this._reconnectAttempts = 0;
      this.reconnecting = false;
      this.reconnectCount++;
      dbg('RECONNECT', `success (total reconnects: ${this.reconnectCount})`);
      ...
      dbg('RECONNECT', `failed: ${err.message}`);
      void this._scheduleReconnect();
    }
  }
