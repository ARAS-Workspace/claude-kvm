...
export class VNCClient extends EventEmitter {
  ...
  _handleARDAuth() {
    ...
    try {
      const generatorBuf = Buffer.alloc(keyLength);
      generatorBuf.writeUInt8(generator, keyLength - 1);

      const dh = crypto.createDiffieHellman(prime, generatorBuf);
      dh.generateKeys();

      const clientPublicKey = Buffer.from(/** @type {Uint8Array} */ (dh.getPublicKey()));
      const sharedSecret = Buffer.from(/** @type {Uint8Array} */ (dh.computeSecret(serverPublicKey)));

      dbg('ARD', `DH shared secret (first 16 bytes): ${sharedSecret.subarray(0, 16).toString('hex')}`);
      ...
      const clientKeyPadded = Buffer.alloc(keyLength, 0);
      if (clientPublicKey.length <= keyLength) {
        clientPublicKey.copy(clientKeyPadded, keyLength - clientPublicKey.length);
      } else {
        clientPublicKey.copy(clientKeyPadded, 0, clientPublicKey.length - keyLength);
      }

      dbg('ARD', `sending clientKey (${clientKeyPadded.length} bytes) + encrypted creds (${encrypted.length} bytes)`);
      this.socket.write(Buffer.concat([clientKeyPadded, encrypted]));
  ...
  _handleServerInit() {
    ...
    dbg('READY', `connected to "${this.serverName}" at ${this.width}x${this.height}`);
  ...
  async screenshot() {
    ...
    dbg('SCREENSHOT', `requesting full framebuffer update (dirty=${this.framebufferDirty})...`);
    ...
    dbg('SCREENSHOT', `converting ${this.width}x${this.height} framebuffer to PNG...`);
    const pngBuffer = await sharp(this.framebuffer, {
      raw: { width: this.width, height: this.height, channels: 4 },
    }).png().toBuffer();

    dbg('SCREENSHOT', `PNG: ${pngBuffer.length} bytes`);
    this.framebufferDirty = false;
